...
#Random.seed!(1234) #random Seed, not needed bur orderly
# using Pkg
# Pkg.add(Pkg.PackageSpec(name="Metalhead", version="0.9.5"))
# Pkg.add(Pkg.PackageSpec(name="MLUtils", version="0.4.5"))
# Pkg.add(Pkg.PackageSpec(name="Flux", version="0.14.25"))
...
for epoch in 1:Epochs
    ...
    # --- BAD EPOCH LOGIC HERE ---
    if acc_test >= best_acc
        best_acc = acc_test
        bad_epochs = 0
        @save "best_resnet18.bson" model classes epoch acc_test
        println("  → New best model saved! acc_test=$(round(acc_test, digits=4))")
    else
        bad_epochs += 1
        println("  → Bad epoch ($bad_epochs in a row)")
    end

    println("epoch $epoch  train_loss=$(round(train_loss,digits=6))  test_loss=$(round(test_loss,digits=6))  acc_train=$(round(acc_train,digits=3))  acc_test=$(round(acc_test,digits=3))")
